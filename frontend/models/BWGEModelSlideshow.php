...
class BWGEModelSlideshow {
  ...
  public function get_image_rows_data($id, $sort_by, $order_by = 'asc', $bwge) {
    global $wpdb;
    ...
    $row = $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'bwge_image WHERE published=1 AND gallery_id="%d" ORDER BY ' . $sort_by . ' ' . $order_by, $id));
    return $row;
  }
