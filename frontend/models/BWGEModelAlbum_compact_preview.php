...
class BWGEModelAlbum_compact_preview {
  ////////////////////////////////////////////////////////////////////////////////////////
  // Events                                                                             //
  ////////////////////////////////////////////////////////////////////////////////////////
  ////////////////////////////////////////////////////////////////////////////////////////
  ...
  public function get_alb_gals_row($id, $albums_per_page, $sort_by, $bwge, $sort_direction = ' ASC ') {
    ...
    $row = $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'bwge_album_gallery WHERE album_id="%d" ORDER BY ' . ($sort_by == "RAND()" ? '' : '`') . $sort_by . ($sort_by == "RAND()" ? '' : '`') . $sort_direction . $limit_str, $id));
    return $row;
  }
  ...
  public function get_album_row_data($id) {
    ...
    if ($row) {
      $row->permalink = $this->bwge_create_post($row->name, $row->slug, "album", $id);
    }
    return $row;
  }
  ...
  public function get_gallery_row_data($id) {
    ...
    if ($row) {
      $row->permalink = $this->bwge_create_post($row->name, $row->slug, "gallery", $id);
    }
    return $row;
  }
  ...
  public function get_image_rows_data($id, $images_per_page, $sort_by, $bwge, $sort_direction = ' ASC ') {
    ...
    $bwge_search = ((isset($_POST['bwge_search_' . $bwge])) ? esc_html($_POST['bwge_search_' . $bwge]) : '');
    if ($bwge_search != '') {
  ...
  public function bwge_create_post($title, $slug, $type, $id) {
    ...
    if (!$bwge_post_id) {    
      ...
      $bwge_post_id->post_name = $slug;
      $bwge_post_id->post_content = $shortecode_string;
      wp_update_post($bwge_post_id);
    }
    $bwge_post_id = get_page_by_title($title, OBJECT, $post_type);      
    return get_permalink($bwge_post_id->ID);
  }
