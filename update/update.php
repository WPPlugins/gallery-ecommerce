...
class WDW_bwge_Update {
  ...
  protected $language_prefix = 'bwge_back';
	protected $plugin_name = 'gallery-ecommerce';
	protected $plugin_url = WD_BWGE_URL;
	protected $parent_menu_slug = 'galleries_bwge';
      ...
      if (count($request_ids) > 0) {
        ...
        if (isset($remote_version['body'])) {
          foreach ($remote_version['body'] as $id => $updated_plugin) {
						if (isset($updated_plugin[0]['version']) && version_compare($plugins[$id]['Version'], $updated_plugin[0]['version'], '<') && strpos(strtolower($updated_plugin[0]['note']), 'important') !== FALSE) {
  ...
  public function get_remote_version($id) {
    $userhash = 'nohash';
		if (file_exists(WD_BWGE_DIR . '/.keep') && is_readable(WD_BWGE_DIR . '/.keep')) {
			$f = fopen(WD_BWGE_DIR.'/.keep', 'r');
			$userhash = fgets($f);
			fclose($f);
		}
						...
						<div class="main-plugin-info">
							...
							<div class="main-plugin_desc-cont">
								...
								<div class="main-plugin-desc main-plugin-desc-info">
									<p><strong><?php echo sprintf(__('Current version %s', $this->language_prefix), $project['Version']); ?></strong></p>
								</div>
								<?php
