...
class BWGEFilemanagerModel {
    ...
    public function get_path_components() {
      ...
      $component['name'] = $WD_BWGE_UPLOAD_DIR;
      $component['path'] = $path;
    ...
    function get_files($sort_by, $sort_order) {
      $icons_dir_path = WD_BWGE_DIR . '/filemanager/images/file_icons';
      ...
      foreach ($file_names as $file_name) {
        ...
        if (is_dir($parent_dir . '/' . $file_name) == TRUE) {
          ...
          $exif = $this->bwge_wp_read_image_metadata( $parent_dir . '/.original/' . $file_name );
    ...
    function get_media_library_files($sort_by, $sort_order) {
      ...
      foreach ($query_images->posts as $image) {
        ...
        if (isset($file_meta['file'])) {
          ...
          $file_bwge_typearray = explode('.', $file_name);
          $file['type'] = strtolower(end($file_bwge_typearray));
    ...
    private function bwge_wp_read_image_metadata( $file ) {
      if (!file_exists($file)) {
        return false;
      }
