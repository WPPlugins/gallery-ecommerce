...
/** 
 * client side analogue is function bwge_spider_display_embed in bwge_embed.js
 *
  ...
  public static function display_embed($embed_type, $file_url, $embed_id = '', $attrs = array()) {
    ...
    switch ($embed_type) {
	  ...
	  case 'EMBED_OEMBED_FACEBOOK_VIDEO':
        $oembed_facebook_html ='<iframe class="bwge_fb_video"';
        if($embed_id!=''){
 ...
 */
  ...
  public static function get_facebook_photo_data($url, $facebook_sdk, $bwge_typeor_id) {
    if($bwge_typeor_id == 'profile') {
  ...
  public static function get_facebook_video_data($video_id, $facebook_sdk) {    
    ...
    try {
      $api_call = '/' . $video_id.'?fields=source,name,created_time,updated_time,from,description,format';						
  ...
  public static function get_facebook_valid_data_for_video($video_data, $content_type) {   			
    ...
    $video = new stdClass();
    $video->name = isset($video_data['name']) ? $video_data['name'] : $video_data['id'];
    $video->filename = isset($video_data['name']) ? $video_data['name'] : $video_data['id'];
    $video->size = '';
  ...
  public static function get_facebook_album_data($album_url, $album_limit) {
    ...
    $second_token = strtok('/');
    if($second_token != 'www.facebook.com') {
      return array("error", "Incorrect url.");
    }
    $album_id = explode('?set=', $album_url); //from album url
    if (!array_key_exists (1, $album_id)) {
      return array("error", "Incorect album url.");
    }
    $album_id = $album_id['1'];
    //explodes section by periods, Album ID is first of the 3 sets of numbers
    $album_id = explode('.', $album_id);
    if (!array_key_exists (1, $album_id)) {
      return array("error", "Incorrect album url.");
    }
    $album_id = $album_id['1'];	
    try {
      $api_call = '/' . $album_id.'/photos?fields=source,link,name,images,from,album&limit=' . $album_limit;						
  ...
  public static function get_facebook_valid_data_for_album($album_data, $content_type) {
    ...
    if(array_key_exists ('data', $album_data) && count($album_data['data']) == 0)
      return array("error", "Album is empty or belongs to another Facebook user");
        
    $files = array();
    for($i=0; $i<count($album_data['data']); $i++) {
      $images = new stdClass();
      $images_size_count = count($album_data['data'][$i]['images']);
        $push = false;	  
      for($j=0; $j<$images_size_count; $j++) {
      if($j==0) {
        $source = $album_data['data'][$i]['images'][$j]['source'];
        //get photo's width and height for resolution
        $width = $album_data['data'][$i]['images'][$j]['width'];
        $height = $album_data['data'][$i]['images'][$j]['height'];
        $resolution = $width . ' x ' . $height . ' px';
        
        //get photo's fbid for filrname feild and for url if embed type is post
        $fbid = $album_data['data'][$i]['id'];          		   
        ...
        $images->name = (isset($album_data['data'][$i]['name']) ? $album_data['data'][$i]['name'] : $fbid );
        ...
        $thumb = $album_data['data'][$i]['images'][$j]['source'];
        $images->thumb = $thumb;
 ...
 */  
  ...
  public static function check_instagram_galleries(){
    ...
    global $wpdb;
    $instagram_galleries = $wpdb->get_results( "SELECT id, gallery_type, gallery_source, update_flag, autogallery_image_number  FROM " . $wpdb->prefix . "bwge_gallery WHERE gallery_type='instagram' OR gallery_type='instagram_post'", OBJECT );
  ...
  public static function check_facebook_galleries() {
    global $wpdb;
    $facebook_galleries = $wpdb->get_results( "SELECT id, gallery_type, gallery_source, update_flag, autogallery_image_number  FROM " . $wpdb->prefix . "bwge_gallery WHERE gallery_type='facebook' OR gallery_type='facebook_post'");
 ...
 */
  ...
  public static function refresh_social_gallery($gallery) {
	...
	if ($is_instagram) {
    $get_access_token = $wpdb->get_results( "SELECT instagram_access_token FROM " . $wpdb->prefix . "bwge_option", OBJECT );
    if(!$get_access_token){
      return array(false, "Cannot get access token from the database");
    }
    $access_token = $get_access_token[0]->instagram_access_token;
    ...
    $images = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "bwge_image WHERE gallery_id='" . $id . "' ", OBJECT);
    ...
    if($images_new[0] == "error") {
      if(!$is_instagram) {
        ...
        if($images_new[1] == 'Unsupported get request') {
          ...
          //$query = 'UPDATE ' .  $wpdb->prefix . 'bwge_gallery SET published=0 WHERE `id`='.$id;
          //$wpdb->query($query);		  
        }
      }
      return array(false, "Cannot get social data"); 
    ...
    foreach ($images_new as $image_new) {
      ...
      if($to_add){
        ...
        $new_image_data = array(
          ...
          'description' => self::bwge_spider_replace4byte($image_new->description),
          'alt' => self::bwge_spider_replace4byte($image_new->name),
    ...
    if($images_count != 0){
      if($to_unpublish){
    		foreach ($images_dated as $image) {
    			$q = 'UPDATE ' .  $wpdb->prefix . 'bwge_image SET published=0, `order` ='.$image['order'].' WHERE `id`='.$image['id'];
          $wpdb->query($q);
    		}
    	}
    	else{
    		foreach ($images_dated as $image) {
          $q = 'UPDATE ' .  $wpdb->prefix . 'bwge_image SET `order` ='.$image['order'].' WHERE `id`='.$image['id'];
    			$wpdb->query($q);
    		}		
    	}
      ...
      foreach ($images_update as $image) {
        $save = $wpdb->update($wpdb->prefix . 'bwge_image', array(
          'order' => $image['order'],
          'slug' => self::bwge_spider_replace4byte($image['slug']),
          'description' => self::bwge_spider_replace4byte($image['description']),
          'alt' => self::bwge_spider_replace4byte($image['alt']),
          'date' => $image['date']
          ), array('id' => $image['id']));
      }
    }
