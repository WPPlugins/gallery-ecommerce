function addPricelist(pricelist){
  ...
  bwge_spider_set_input_value('ajax_task', 'set_image_pricelist');
  bwge_spider_ajax_save('galleries_form');
  jQuery(document).trigger('selectPricelist');                                                 
}
function bwge_remove_pricelist(obj){
  jQuery("#remove_pricelist").val(jQuery(obj).attr("data-image-id"));
  jQuery("#pricelist_id_" + jQuery(obj).attr("data-pricelist-id") ).val("");  
  bwge_spider_set_input_value('ajax_task', 'remove_image_pricelist');
  bwge_spider_ajax_save('galleries_form');
  ...
  jQuery.ajax({
    ...
    success: function(html) {
      ...
      if (array[0] == "The slug must be unique.") {
        ...
        var func1="bwge_spider_set_input_value('task', 'delete');";
        var func2="bwge_spider_set_input_value('current_id', "+tag_id+");";
        var func3="bwge_spider_form_submit('event', 'tags_form')";
...
var bwge_save_count = 50;
function bwge_spider_ajax_save(form_id, tr_group) {
  ...
  var bwge_nonce = jQuery("#bwge_nonce").val();
  var name = jQuery("#name").val();
  var slug = jQuery("#slug").val();

  var gallery_bwge_typeinput = jQuery("#gallery_type").val();

  var gallery_source = (gallery_bwge_typeinput == 'facebook') ? jQuery("#facebook_gallery_source").val() : jQuery("#gallery_source").val(); 
  var autogallery_image_number = (gallery_bwge_typeinput == 'facebook') ? jQuery("#facebook_gallery_image_limit").val() : jQuery("#autogallery_image_number").val();
  var update_flag = (gallery_bwge_typeinput == 'facebook') ? jQuery("input[name=facebook_update]:checked").val() : jQuery("input[name=update_flag]:checked").val(); 
  var gallery_content_type = (gallery_bwge_typeinput == 'facebook') ? jQuery("input[name=facebook_content_type]:checked").val() : jQuery("input[name=instagram_post_gallery]:checked").val();
  var gallery_type = gallery_bwge_typeinput + (gallery_content_type == 1 ? "_post" : "");
  var gallery_bwge_typeold = jQuery("#gallery_bwge_typeold").val();
  ...
  document.cookie = 'bwge_image_asc_or_desc=' + image_asc_or_desc;
  document.cookie = 'bwge_image_order_by=' + image_order_by;
  
  var image_current_id = jQuery("#image_current_id").val();
  ids_array = ids_string.split(",");
  var tr_count = ids_array.length;
  if (tr_count > bwge_save_count) {
    // Remove items form begin and end of array.
    ids_array.splice(tr_group * bwge_save_count, ids_array.length);
    ids_array.splice(0, (tr_group - 1) * bwge_save_count);
  ...
  var post_data = {};
  post_data["bwge_nonce"] = bwge_nonce;
  post_data["name"] = name;
  post_data["slug"] = slug;

  post_data["gallery_type"] = gallery_type;
  post_data["gallery_bwge_typeold"] = gallery_bwge_typeold;
  ...
  for (var i in ids_array) {
    if (ids_array.hasOwnProperty(i) && ids_array[i]) {
      ...
      post_data["input_filebwge_type" + ids_array[i]] = jQuery("#input_filebwge_type" + ids_array[i]).val();
  ...
  jQuery.post(
    ...
    if (tr_count > bwge_save_count * tr_group) {
      bwge_spider_ajax_save(form_id, ++tr_group);
      return;
    }
    else {
      ...
      if (ajax_task == 'ajax_apply') {
        ...
        if(not_set_items.length > 0){
          jQuery('#message_div').html("<strong><p>" + bwge_objectL10B.saved + " Selected pricelist item longest dimension greater than some original images dimensions.</p></strong>");
        }
        else{
          jQuery('#message_div').html("<strong><p>" + bwge_objectL10B.saved + "</p></strong>");
        }
        jQuery('#message_div').show();
      }
      else if (ajax_task == 'recover') {
        jQuery('#draganddrop').html("<strong><p>" + bwge_objectL10B.recoverd + "</p></strong>");
        jQuery('#draganddrop').show();
      }
      else if (ajax_task == 'image_publish') {
        jQuery('#draganddrop').html("<strong><p>" + bwge_objectL10B.published + ".</p></strong>");
        jQuery('#draganddrop').show();
      }
      else if (ajax_task == 'image_unpublish') {
        jQuery('#draganddrop').html("<strong><p>" + bwge_objectL10B.unpublished + "</p></strong>");
        jQuery('#draganddrop').show();
      }
      else if (ajax_task == 'image_delete') {
        jQuery('#draganddrop').html("<strong><p>" + bwge_objectL10B.deleted + "</p></strong>");
        jQuery('#draganddrop').show();
      }
        ...
        jQuery('#draganddrop').html("<strong><p>" + bwge_objectL10B.one_item + "</p></strong>");
        jQuery('#draganddrop').show();
      }
        ...
        jQuery('#draganddrop').html("<strong><p>" + bwge_objectL10B.resized + "</p></strong>");
        jQuery('#draganddrop').show();
      }
      else if (ajax_task == 'image_set_watermark') {
        jQuery('#draganddrop').html("<strong><p>" + bwge_objectL10B.watermark_set + "</p></strong>");
        jQuery('#draganddrop').show();
      }
        ...
        jQuery('#draganddrop').html("<strong><p>" + bwge_objectL10B.reset + "</p></strong>");
        jQuery('#draganddrop').show();
      }
		...
		var msg = jQuery(data).find(".bwge_msg").html();
...
function bwge_spider_run_checkbox() {
...
function bwge_spider_show_hide_weights() {
  if (jQuery("#show_hide_weights").val() == bwge_objectL10B.bwge_show_order) {
    jQuery(".connectedSortable").css("cursor", "default");
    jQuery("#tbody_arr").find(".handle").hide(0);
    jQuery("#th_order").show(0);
    jQuery("#tbody_arr").find(".bwge_spider_order").show(0);
    jQuery("#show_hide_weights").val(bwge_objectL10B.bwge_hide_order);
    ...
    jQuery("#tbody_arr").sortable({
      ...
      update:function (event, tr) {
        ...
        var i = page_number;
        jQuery('.bwge_spider_order').each(function (e) {
    ...
    jQuery("#tbody_arr").find(".bwge_spider_order").hide(0);
    jQuery("#show_hide_weights").val(bwge_objectL10B.bwge_show_order);
  }
}

// Check all items.
function bwge_spider_check_all_items() {
  bwge_spider_check_all_items_checkbox();
  // if (!jQuery('#check_all').attr('checked')) {
    jQuery('#check_all').trigger('click');
  // }
  
}
function bwge_spider_check_all_items_checkbox() {
  if (jQuery('#check_all_items').attr('checked')) {
    ...
    if (items_count) {
      jQuery('#draganddrop').html("<strong><p>" + bwge_objectL10B.selected + ' ' + items_count + ' ' + bwge_objectL10B.item + (items_count > 1 ? "s" : "") + ".</p></strong>");
      jQuery('#draganddrop').show();
    }
  }
}

function bwge_spider_check_all(current) {
...
function bwge_spider_uploader(button_id, input_id, delete_id, img_id) {
    ...
    window.send_to_editor = function (html) {
      if (formfield) {
        ...
        if (!fileurl) {
          ...
          if (img_id != '') {
            alert(bwge_objectL10B.bwge_select_image);
            tb_remove();
            return;
          }
          ...
          if (img_id == '') {
            alert(bwge_objectL10B.bwge_field_required);
            tb_remove();
            return;
          }
          window.parent.document.getElementById(input_id).value = fileurl;
...
function bwge_spider_remove_url(button_id, input_id, delete_id, img_id) {
  if (typeof img_id == 'undefined') {
    img_id = '';
  }
...
function bwge_spider_reorder_items(tbody_id) {
  jQuery("#" + tbody_id).sortable({
    handle:".connectedSortable",
    connectWith:".connectedSortable",
    update:function (event, tr) {
      bwge_spider_sortt(tbody_id);
    }
  });
}

function bwge_spider_sortt(tbody_id) {
  var str = "";
  var counter = 0;
...
function bwge_spider_remove_row(tbody_id, event, obj) {
  var span = obj;
...
function bwge_spider_jslider(idtaginp) {
  jQuery(function () {
...
function bwge_spider_get_items(e) {
  if (e.preventDefault) {
    e.preventDefault();
  }
  else {
  ...
  window.parent.bwge_add_items(trackIds, titles, types);
}

function bwge_check_checkboxes(bwge_alert) {
  if(typeof bwge_alert == 'undefined'){
    bwge_alert = false;
  }  
...
function bwge_get_tags(image_id, e) { 
  if (e.preventDefault) {
    e.preventDefault();
  }
  else {
  ...
  window.parent.bwge_add_tag(image_id, tagIds, titles);
}

function bwge_add_tag(image_id, tagIds, titles) {
  if (image_id == '0') {
    var flag = false;
    var ids_string = jQuery("#ids_string").val();
    ids_array = ids_string.split(",");
      ...
      if (jQuery("#check_" + ids_array[i]).attr('checked') == 'checked' || flag) {
        ...
        for (i = 0; i < tagIds.length; i++) {
          if (tags_array.indexOf(tagIds[i]) == -1) {
            ...
            tag_delete_span.setAttribute('class', "bwge_spider_delete_img_small");
            tag_delete_span.setAttribute('onclick', "bwge_remove_tag('" + tagIds[i] + "', '" + image_id + "')");
...
function bwge_remove_tag(tag_id, image_id) {
...
function preview_watermark() {
  setTimeout(function() {
    watermark_type = window.parent.document.getElementById('watermark_bwge_typetext').checked;
    if (watermark_type) {
      ...
      stringHTML = (watermark_link ? '<a href="' + watermark_link + '" target="_blank" style="text-decoration: none;' : '<span style="cursor:default;') + 'margin:4px;font-size:' + watermark_font_size + 'px;font-family:' + watermark_font + ';color:#' + watermark_color + ';opacity:' + (watermark_opacity / 100) + ';" class="bwge_non_selectable">' + watermark_text + (watermark_link ? '</a>' : '</span>');
      document.getElementById("preview_watermark").innerHTML = stringHTML;
    }
    watermark_type = window.parent.document.getElementById('watermark_bwge_typeimage').checked;
    if (watermark_type) {
      ...
      document.getElementById("preview_watermark").style.verticalAlign = watermark_position[0];
      document.getElementById("preview_watermark").style.textAlign = watermark_position[1];
      stringHTML = (watermark_link ? '<a href="' + watermark_link + '" target="_blank">' : '') + '<img class="bwge_non_selectable" src="' + watermark_url + '" style="margin:0 4px 0 4px;max-width:' + watermark_width + 'px;max-height:' + watermark_height + 'px;opacity:' + (watermark_opacity / 100) + ';" />' + (watermark_link ? '</a>' : '');
      document.getElementById("preview_watermark").innerHTML = stringHTML;
    }
  }, 50);
}
...
function preview_built_in_watermark() {
  setTimeout(function(){
  watermark_type = window.parent.document.getElementById('built_in_watermark_bwge_typetext').checked;
  if (watermark_type) {
    ...
    watermark_font = 'bwge_' + document.getElementById('built_in_watermark_font').value.replace('.TTF', '').replace('.ttf', '');
    ...
    stringHTML = '<span style="cursor:default;margin:4px;font-size:' + watermark_font_size + 'px;font-family:' + watermark_font + ';color:#' + watermark_color + ';opacity:' + (watermark_opacity / 100) + ';" class="bwge_non_selectable">' + watermark_text + '</span>';
    document.getElementById("preview_built_in_watermark").innerHTML = stringHTML;
  }
  watermark_type = window.parent.document.getElementById('built_in_watermark_bwge_typeimage').checked;
  if (watermark_type) {
    ...
    document.getElementById("preview_built_in_watermark").style.verticalAlign = watermark_position[0];
    document.getElementById("preview_built_in_watermark").style.textAlign = watermark_position[1];
    stringHTML = '<img class="bwge_non_selectable" src="' + watermark_url + '" style="margin:0 4px 0 4px;max-width:95%;width:' + watermark_size + '%;" />';
    document.getElementById("preview_built_in_watermark").innerHTML = stringHTML;
  }
  }, 50);
}
...
function bwge_built_in_watermark(watermark_type) {
...
function bwge_change_option_type(type) {
  type = (type == '' ? 1 : type);
  document.getElementById('type').value = type;
  for (var i = 1; i <= 9; i++) {
    if (i == type) {
      document.getElementById('div_content_' + i).style.display = 'block';
      document.getElementById('div_' + i).style.background = '#0A7393';
      document.getElementById('div_' + i).style.color = '#fff';
    }
    else {
      document.getElementById('div_content_' + i).style.display = 'none';
      document.getElementById('div_' + i).style.background = '#fff';
      document.getElementById('div_' + i).style.color = '#0A7393';
    }
  }
  document.getElementById('display_panel').style.display = 'inline-block';
}

function bwge_inputs() {
...
function bwge_show_hide_roles() {
  if(jQuery("select[name='permissions']").val() == "Administrator"){
    jQuery(".bwge_roles").hide();
  }
  else{
    jQuery(".bwge_roles").show();	
  }
}
...
function bwge_popup_fullscreen(num) {
  if (num) {
...
function bwge_change_album_view_type(type) {
  if (type == 'thumbnail') {
...
function bwge_spider_check_isnum(e) {
...
function bwge_change_theme_type(type) {
  ...
  jQuery("#" + type).show();
  jQuery("#bwge_typemenu").show();
  jQuery(".bwge_spider_fieldset").show();
  jQuery("#current_type").val(type);

  jQuery("#bwge_typeThumbnail").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeMasonry").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeMosaic").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeSlideshow").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeCompact_album").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeExtended_album").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeMasonry_album").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeImage_browser").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeBlog_style").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeCarousel").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeLightbox").attr("style", "background-color: #fff;color: #0A7393;");
  jQuery("#bwge_typeNavigation").attr("style", "background-color: #fff;color: #0A7393;");
  
  jQuery("#bwge_type" + type).attr("style", "background-color: #0A7393;color: #fff;");

}

function bwge_gallery_type(instagram_client_id) {
  var response = true;
  var value = jQuery('#gallery_type').val();
  response = bwge_change_gallery_type(value, 'change',instagram_client_id);
  return response;
}
function bwge_gallery_update_flag(){
  var update_flag = jQuery('#tr_update_flag input[name=update_flag]:checked').val();
  if(update_flag == ''){
    jQuery('.bwge_spider_delete_button').show();
    /*
    ...
    */
    ...
    jQuery('.bwge_spider_delete_button').hide();
    /*
...
bwge_gallery_change_update_flag = jQuery(function () {
...
/*
   bwge_typeto_set:'' or 'instagram'
*/
function bwge_change_gallery_type(bwge_typeto_set, warning_type, instagram_client_id) {
  
  warning_type = (typeof warning_type === "undefined") ? "default" : warning_type;

  if(bwge_typeto_set == 'instagram'){

    if(!bwge_check_gallery_empty(true, true)){
      return false;
    }
    if(instagram_client_id == ''){ 
      alert(bwge_objectL10B.bwge_access_token);
    ...
    jQuery('.bwge_spider_delete_button').hide();
    jQuery('#bwge_spider_setwatermark_button').hide();
    jQuery('#bwge_spider_resize_button').hide();
    jQuery('#bwge_spider_reset_button').hide();
    jQuery('#content-add_media').hide();
    jQuery('#show_add_embed').hide();
    jQuery('#show_bulk_embed').hide();
  }
  else if(bwge_typeto_set == 'facebook') {   
	if(!bwge_check_gallery_empty(true, true)) {
      return false;
    }
    ...
    if(tr_count != 0){
      switch(warning_type) {
        case 'default':
          var allowed = confirm("This action will reset gallery type to mixed and will save that choice. You cannot undo it.");
          break;
        case 'change':
    ...
    jQuery('.bwge_spider_delete_button').show();
    jQuery('#bwge_spider_setwatermark_button').show();
    jQuery('#bwge_spider_resize_button').show();
    jQuery('#bwge_spider_reset_button').show();
...
function bwge_check_instagram_gallery_input(instagram_client_id, from_popup){
  from_popup = typeof from_popup !== 'undefined' ? from_popup : false;
  var is_error = false;
  if(from_popup){
    if(bwge_spider_check_required('popup_instagram_gallery_source', 'Instagram user URL')){
      is_error = true;
    }
    if(instagram_client_id == ''){ 
      alert(bwge_objectL10B.bwge_access_token);
    ...
    if(jQuery('#popup_instagram_image_number').val() > 33 ||jQuery('#popup_instagram_image_number').val() < 1 ){ 
      ...
      alert(bwge_objectL10B.bwge_post_number);
	  ...
	  if (bwge_is_instagram_gallery()){
	    if(bwge_spider_check_required('gallery_source', 'Instagram user URL')){
	      is_error = true;
	    }
	    if(instagram_client_id == ''){ 
	      alert(bwge_objectL10B.bwge_access_token);
...
function bwge_is_instagram_gallery() {
...
function bwge_check_gallery_empty(reset, message){
  ...
  if(tr_count != 0){
    if(reset){
      if(message){
        alert(bwge_objectL10B.bwge_not_empty);
      }
      jQuery('#gallery_type').val('');
      jQuery('#tr_instagram_post_gallery').hide();
      jQuery('#tr_gallery_source').hide();
...
function bwge_convert_seconds(seconds) {
...
function bwge_convert_date(date, separator) {
...
/* EMBED handling */
function bwge_get_embed_info(input_id) {
  jQuery('#opacity_div').show();
  jQuery('#loading_div').show();
  ...
  if (!url) {
    alert(bwge_objectL10B.bwge_enter_url);
    return '';
  }
  var filesValid = [];
  var data = {
    'action': 'bwge_addEmbed',
    'URL_to_embed': url,
    'async':true
  };
  ...
  jQuery.post(ajax_url, data, function(response) {
    if(response == false){
      alert(bwge_objectL10B.bwge_cannot_response);
      jQuery('#opacity_div').hide();
      jQuery('#loading_div').hide();
      return '';
    }
      ...
      if(index_start == -1 || index_end == -1){
        alert(bwge_objectL10B.bwge_something_wrong);
        jQuery('#opacity_div').hide();
        jQuery('#loading_div').hide();
      ...
      if(typeof response_JSON[0] !== 'undefined'){
        alert(bwge_objectL10B.bwge_error + jQuery.parseJSON(response)[1]);
        jQuery('#opacity_div').hide();
        jQuery('#loading_div').hide();
        return '';
      }
...
/*bulk embed handling*/
function bwge_bulk_embed(from, instagram_client_id){
  switch (from) {
    case 'instagram' : {
