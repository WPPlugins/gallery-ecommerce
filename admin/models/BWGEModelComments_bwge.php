...
class BWGEModelComments_bwge {
  ...
  public function get_rows_data() {
    ...
    $rows = $wpdb->get_results("SELECT t1.*, t2.thumb_url, t2.alt, t2.filetype FROM " . $wpdb->prefix . "bwge_image_comment as t1 INNER JOIN " . $wpdb->prefix . "bwge_image as t2 on t1.image_id=t2.id " . $where . $order_by . " LIMIT " . $limit . ",".$this->per_page);
    return $rows;
  }

  public function get_image_for_comments_table($image_id) {
    global $wpdb;
    $preview_image = $wpdb->get_var($wpdb->prepare("SELECT thumb_url FROM " . $wpdb->prefix . "bwge_image WHERE id='%d'", $image_id));
    return $preview_image;
  }

  public function get_image_filename_for_comments_table($image_id) {
    global $wpdb;
    $preview_image_filename = $wpdb->get_var($wpdb->prepare("SELECT filename FROM " . $wpdb->prefix . "bwge_image WHERE id='%d'", $image_id));
    return $preview_image_filename;
  }

  public function get_galleries() {
    global $wpdb;
    $rows_object = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "bwge_gallery WHERE published=1");
  ...
  public function get_images($gal_id) {
    ...
    $rows_object = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "bwge_image WHERE published=1" . $where);
    $rows[0] = 'All';
    if ($rows_object) {
      foreach ($rows_object as $row_object) {
  ...
  public function page_nav() {
    ...
    $where .= ((isset($_POST['search_select_gal_value']) && ((int) $_POST['search_select_gal_value'])) ? ' AND t2.gallery_id=' . (int) $_POST['search_select_gal_value'] : '');
    $where .= ((isset($_POST['search_select_value']) && ((int) $_POST['search_select_value'])) ? ' AND t1.image_id=' . (int) $_POST['search_select_value'] : '');
    $query = "SELECT COUNT(*) FROM " . $wpdb->prefix . "bwge_image_comment as t1 INNER JOIN " . $wpdb->prefix . "bwge_image as t2 on t1.image_id=t2.id " . $where;
