...
class BWGEModelAlbums_bwge {
  ...
  public function get_rows_data() {
    global $wpdb;
    if (!current_user_can('manage_options') && $wpdb->get_var("SELECT album_role FROM " . $wpdb->prefix . "bwge_option")) {
    ...
    $query = "SELECT * FROM " . $wpdb->prefix . "bwge_album " . $where . $order_by . " LIMIT " . $limit . ",".$this->per_page;
  ...
  public function get_row_data($id) {
    ...
    if ($id != 0) {
      ...
      $row = $wpdb->get_row($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'bwge_album ' . $where . ' AND id="%d"', $id));
    }
    else {
      $row = new stdClass();
      $row->id = 0;
  ...
  public function get_albums_galleries_rows_data($album_id) {
    ...
    $row = $wpdb->get_results("SELECT t1.id, t2.name, t2.slug, t1.is_album, t1.alb_gal_id, t1.order FROM " . $wpdb->prefix . "bwge_album_gallery as t1 INNER JOIN " . $wpdb->prefix . "bwge_album as t2 on t1.alb_gal_id = t2.id where t1.is_album='1'" . $where . " AND t1.album_id='" . $album_id . "' union SELECT t1.id, t2.name, t2.slug, t1.is_album, t1.alb_gal_id, t1.order FROM " . $wpdb->prefix . "bwge_album_gallery as t1 INNER JOIN " . $wpdb->prefix . "bwge_gallery as t2 on t1.alb_gal_id = t2.id where t1.is_album='0'" . $where . " AND t1.album_id='" . $album_id . "' ORDER BY `order`");
    return $row;
  }
  ...
  public function page_nav() {
    ...
    $query = "SELECT COUNT(*) FROM " . $wpdb->prefix . "bwge_album " . $where;
