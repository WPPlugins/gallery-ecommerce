...
class BWGEModelGalleries_bwge {
  ...
  public function get_image_rows_data($gallery_id) {
    global $wpdb;
    if (!current_user_can('manage_options') && $wpdb->get_var("SELECT image_role FROM " . $wpdb->prefix . "bwge_option")) {
    ...
    $image_order_by = ((isset($_POST['image_order_by']) && esc_html(stripslashes($_POST['image_order_by'])) != '') ? esc_html(stripslashes($_POST['image_order_by'])) : ((isset($_COOKIE['bwge_image_order_by']) && esc_html(stripslashes($_COOKIE['bwge_image_order_by'])) != '') ? esc_html(stripslashes($_COOKIE['bwge_image_order_by'])) : 'order'));
  ...
  public function get_tag_rows_data($image_id) {
    global $wpdb;
    $rows = $wpdb->get_results($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "terms AS table1 INNER JOIN " . $wpdb->prefix . "bwge_image_tag AS table2 ON table1.term_id=table2.tag_id WHERE table2.image_id='%d' ORDER BY table2.tag_id", $image_id));
    return $rows;
  }

  public function get_rows_data() {
    global $wpdb;
    if (!current_user_can('manage_options') && $wpdb->get_var("SELECT gallery_role FROM " . $wpdb->prefix . "bwge_option")) {
      $where = " WHERE author=" . get_current_user_id();
    }
    else {
      $where = " WHERE author>=0 ";
    ...
    $query = "SELECT * FROM " . $wpdb->prefix . "bwge_gallery " . $where . $order_by . " LIMIT " . $limit . ",".$this->per_page;
  ...
  public function get_row_data($id) {
    ...
    if ($id != 0) {
      ...
      $row = $wpdb->get_row($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'bwge_gallery ' . $where . ' AND id="%d"', $id));
    }
    else {
      $row = new stdClass();
      $row->id = 0;
  ...
  public function page_nav() {
    ...
    $query = "SELECT COUNT(*) FROM " . $wpdb->prefix . "bwge_gallery " . $where;
  ...
  public function image_page_nav($gallery_id) {
    ...
    $query = "SELECT COUNT(*) FROM " . $wpdb->prefix . "bwge_image WHERE gallery_id='" . $gallery_id . "' " . $where;
    $total = $wpdb->get_var($query);
    $page_nav['total'] = $total;
    if (isset($_POST['page_number']) && $_POST['page_number']) {
      $limit = ((int) $_POST['page_number'] - 1) * $this->per_page;
  ...
  public function get_option_row_data() {
    ...
    $row = $wpdb->get_var($wpdb->prepare("SELECT COUNT(filename) FROM " . $wpdb->prefix . "bwge_image " . $where . " AND gallery_id='%d'", $gallery_id));
    return $row;
  }
