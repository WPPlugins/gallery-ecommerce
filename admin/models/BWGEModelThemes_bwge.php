...
class BWGEModelThemes_bwge {
  ...
  public function get_rows_data() {
    ...
    $query = "SELECT * FROM " . $wpdb->prefix . "bwge_theme " . $where . $order_by . " LIMIT " . $limit . ",".$this->per_page;
    ...
    if ($id != 0) {
      ...
      if ($reset) {
        if (!$row->default_theme) {
          $row_id = $row->id;
          $row_name = $row->name;
          $row = $wpdb->get_row($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'bwge_theme WHERE default_theme="%d"', 1));
          $row->id = $row_id;
          $row->name = $row_name;
          ...
          $row->slideshow_ecommerce_icon_color = '363434';
          $row->slideshow_ecommerce_icon_size = '30';
          $row->slideshow_ecommerce_icon_pos = 'top_left';
          ...
          $row->carousel_ecommerce_icon_color = '363434';
          $row->carousel_ecommerce_icon_size = '30';
          $row->carousel_ecommerce_icon_pos = 'top_left';
  ...
  public function page_nav() {
    global $wpdb;
    $where = ((isset($_POST['search_value']) && (esc_html($_POST['search_value']) != '')) ? 'WHERE name LIKE "%' . esc_html($_POST['search_value']) . '%"'  : '');
    $query = "SELECT COUNT(*) FROM " . $wpdb->prefix . "bwge_theme " . $where;
