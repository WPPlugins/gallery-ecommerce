...
class BWGEControllerThemes_bwge {
  ////////////////////////////////////////////////////////////////////////////////////////
  // Events                                                                             //
  ////////////////////////////////////////////////////////////////////////////////////////
  ////////////////////////////////////////////////////////////////////////////////////////
  ...
  public function execute() {
    ...
    if($task != ''){
      if(!BWGELibrary::verify_nonce('themes_bwge')){
  ...
  public function display() {
    require_once WD_BWGE_DIR . "/admin/models/BWGEModelThemes_bwge.php";
    $model = new BWGEModelThemes_bwge();

    require_once WD_BWGE_DIR . "/admin/views/BWGEViewThemes_bwge.php";
    $view = new BWGEViewThemes_bwge($model);
    $view->display();
  }
  ...
  public function reset() {
    ...
    $view = new BWGEViewThemes_bwge($model);
    $id = BWGELibrary::get('current_id', 0);
    echo BWGELibrary::message('Changes must be saved.', 'error');
    $view->edit($id, TRUE);
  }
  ...
  public function save() {
    ...
    $query_url = wp_nonce_url( admin_url('admin.php'), 'themes_bwge', 'bwge_nonce' );
  ...
  public function apply() {
    $message = $this->save_db();
    global $wpdb;
    $id = (int) $wpdb->get_var('SELECT MAX(`id`) FROM ' . $wpdb->prefix . 'bwge_theme');
    $current_id = BWGELibrary::get('current_id', $id);
    $page = BWGELibrary::get('page');
    $current_type = BWGELibrary::get('current_type', 'Thumbnail');
    $query_url = wp_nonce_url( admin_url('admin.php'), 'themes_bwge', 'bwge_nonce' );
  ...
  public function save_db() {
    ...
    $slideshow_ecommerce_icon_color = (isset($_POST['slideshow_ecommerce_icon_color']) ?  esc_html(stripslashes( $_POST['slideshow_ecommerce_icon_color'])) : '363434');
    $slideshow_ecommerce_icon_size = (isset($_POST['slideshow_ecommerce_icon_size']) ?  esc_html(stripslashes( $_POST['slideshow_ecommerce_icon_size'])) : '30');
    $slideshow_ecommerce_icon_pos = (isset($_POST['slideshow_ecommerce_icon_pos']) ?  esc_html(stripslashes( $_POST['slideshow_ecommerce_icon_pos'])) : 'top_left');
    ...
    $carousel_ecommerce_icon_color = (isset($_POST['carousel_ecommerce_icon_color']) ?  esc_html(stripslashes( $_POST['carousel_ecommerce_icon_color'])) : '363434');
    $carousel_ecommerce_icon_size = (isset($_POST['carousel_ecommerce_icon_size']) ?  esc_html(stripslashes( $_POST['carousel_ecommerce_icon_size'])) : '30');
    $carousel_ecommerce_icon_pos = (isset($_POST['carousel_ecommerce_icon_pos']) ?  esc_html(stripslashes( $_POST['carousel_ecommerce_icon_pos'])) : 'top_left');
    ...
    $themes = array(
      ...
      'slideshow_ecommerce_icon_color' => $slideshow_ecommerce_icon_color,
      'slideshow_ecommerce_icon_size' => $slideshow_ecommerce_icon_size,
      'slideshow_ecommerce_icon_pos' => $slideshow_ecommerce_icon_pos,
      ...
      'carousel_ecommerce_icon_color' => $carousel_ecommerce_icon_color,
      'carousel_ecommerce_icon_size' => $carousel_ecommerce_icon_size,
      'carousel_ecommerce_icon_pos' => $carousel_ecommerce_icon_pos,
    ...
    if ($id != 0) {
      $save = $wpdb->update($wpdb->prefix . 'bwge_theme', array(
        'name' => $name,
        'options' => $themes,
        'default_theme' => $default_theme,
      ), array('id' => $id));
      ...
      $save = $wpdb->insert($wpdb->prefix . 'bwge_theme', array(
        'name' => $name,
        'options' => $themes,
        'default_theme' => $default_theme,
        ));
  ...
  public function delete($id) {
    global $wpdb;
    $isDefault = $wpdb->get_var('SELECT default_theme FROM ' . $wpdb->prefix . 'bwge_theme WHERE id=' . $id);
    if ($isDefault) {
      echo BWGELibrary::message("You can't delete default theme", 'error');
    }
    else {
      $query = $wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_theme WHERE id="%d"', $id);
      if ($wpdb->query($query)) {
  ...
  public function delete_all() {
    ...
    $flag = FALSE;
    $isDefault = FALSE;
    $tag_ids_col = $wpdb->get_col('SELECT id FROM ' . $wpdb->prefix . 'bwge_theme');
    foreach ($tag_ids_col as $tag_id) {
      if (isset($_POST['check_' . $tag_id])) {
        $isDefault = $wpdb->get_var('SELECT default_theme FROM ' . $wpdb->prefix . 'bwge_theme WHERE id=' . $tag_id);
        if ($isDefault) {
          $message = 4;
        }
        else {
          $flag = TRUE;
          $query = $wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_theme WHERE id="%d"', $tag_id);
          $wpdb->query($query);
        }
      }
    }
  ...
  public function setdefault($id) {
    global $wpdb;
    $save = $wpdb->update($wpdb->prefix . 'bwge_theme', array('default_theme' => 0), array('default_theme' => 1));
    $save = $wpdb->update($wpdb->prefix . 'bwge_theme', array('default_theme' => 1), array('id' => $id));
    if ($save !== FALSE) {
      $message = 7;
    }
    else {
