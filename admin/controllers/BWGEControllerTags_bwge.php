...
class BWGEControllerTags_bwge {
  ...
  public function execute() {
    ...
    if($task != ''){
      if(!BWGELibrary::verify_nonce('tags_bwge')){
  ...
  public function display() {
    require_once WD_BWGE_DIR . "/admin/models/BWGEModelTags_bwge.php";
    $model = new BWGEModelTags_bwge();

    require_once WD_BWGE_DIR . "/admin/views/BWGEViewTags_bwge.php";
    $view = new BWGEViewTags_bwge($model);
    $view->display();
  }
  ...
  public function save() {
    ...
    $query_url = wp_nonce_url( admin_url('admin.php'), 'tags_bwge', 'bwge_nonce' );
  ...
  public function save_tag() {
    ...
    $name = $this->bwge_get_unique_name($name, 0);
    $slug = ((isset($_POST['slug']) && (esc_html($_POST['slug']) != '')) ? esc_html(stripslashes($_POST['slug'])) : $name);
    $slug = $this->bwge_get_unique_slug($slug, 0);
	  $slug = sanitize_title($slug);
    if ($name) {
      $save = wp_insert_term($name, 'bwge_tag', array(
        'description'=> '',
        'slug' => $slug,
        'parent' => 0
        )
  ...
  function edit_tag() {
    ...
    if ($name) {
      ...
      $save = wp_update_term($id, 'bwge_tag', array(
        'name' => $name,
        'slug' => $slug
      ));
      
      /*update data in corresponding posts*/
      $query2 = "SELECT ID, post_content FROM ".$wpdb->posts." WHERE post_type = 'bwge_tag' ";
  ...
  public function edit_tags() {
    global $wpdb;
    $flag = FALSE;
    $rows = get_terms('bwge_tag', array('orderby' => 'count', 'hide_empty' => 0));
    $name = ((isset($_POST['tagname'])) ? esc_html(stripslashes($_POST['tagname'])) : '');
    $name = $this->bwge_get_unique_name($name, 0);
    $slug = ((isset($_POST['slug']) && (esc_html($_POST['slug']) != '')) ? esc_html(stripslashes($_POST['slug'])) : $name);
    $slug = $this->bwge_get_unique_slug($slug, 0);
  ...
  public function delete($id) {
    global $wpdb;
    wp_delete_term($id, 'bwge_tag');
    $query = $wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE tag_id="%d"', $id);
  ...
  public function delete_all() {
    ...
    foreach ($tag_ids_col as $tag_id) {
      if (isset($_POST['check_' . $tag_id])) {
        wp_delete_term($tag_id, 'bwge_tag');
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE tag_id="%d"', $tag_id));
        $flag = TRUE;
      }
    }
