...
class BWGEControllerGalleries_bwge {
  ////////////////////////////////////////////////////////////////////////////////////////
  // Events                                                                             //
  ////////////////////////////////////////////////////////////////////////////////////////
  ////////////////////////////////////////////////////////////////////////////////////////
  ...
  public function execute() {
    ...
    if ($task != '') {
      if (!BWGELibrary::verify_nonce('galleries_bwge')) {
  ...
  public function display() {
    require_once WD_BWGE_DIR . "/admin/models/BWGEModelGalleries_bwge.php";
    $model = new BWGEModelGalleries_bwge();

    require_once WD_BWGE_DIR . "/admin/views/BWGEViewGalleries_bwge.php";
    $view = new BWGEViewGalleries_bwge($model);
  ...
  public function save_order_images($gallery_id) {
    global $wpdb;
    $imageids_col = $wpdb->get_col($wpdb->prepare('SELECT id FROM ' . $wpdb->prefix . 'bwge_image WHERE `gallery_id`="%d"', $gallery_id));
    if ($imageids_col) {
      foreach ($imageids_col as $imageid) {
        if (isset($_POST['order_input_' . $imageid])) {
          ...
          $order_values[$imageid] = (int) $wpdb->get_var($wpdb->prepare('SELECT `order` FROM ' . $wpdb->prefix . 'bwge_image WHERE `id`="%d"', $imageid));
        }
      }
      asort($order_values);
      $i = 1;
      foreach ($order_values as $key => $order_value) {
        $wpdb->update($wpdb->prefix . 'bwge_image', array('order' => $i), array('id' => $key));
        $i++;
      }
    }
  }
  ...
  public function ajax_search() {
    if (isset($_POST['ajax_task'])) {
      ...
      if (!isset($_POST['current_id']) || (esc_html(stripslashes($_POST['current_id'])) == 0) || (esc_html(stripslashes($_POST['current_id'])) == '')) {
        ...
        $_POST['current_id'] = (int) $wpdb->get_var('SELECT MAX(`id`) FROM ' . $wpdb->prefix . 'bwge_gallery');
      }
    }
    $this->save_image_db();
  ...
  public function save_tags_if_select_all($tag_ids) {
    ...
    $query_image = $wpdb->prepare('SELECT id FROM ' . $wpdb->prefix . 'bwge_image WHERE gallery_id="%d"', $gal_id);
    ...
    foreach ($image_id_array as $image_id) {
      ...
      foreach ($tag_ids_array as $tag_id) {
        if ($tag_id) {		
          $exist_tag = $wpdb->get_var($wpdb->prepare('SELECT id FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE tag_id="%d" AND image_id="%d" AND gallery_id="%d"', $tag_id,$image_id->id, $gal_id));
          if ($exist_tag == NULL) {
            $save = $wpdb->insert($wpdb->prefix . 'bwge_image_tag', array(
              'tag_id' => $tag_id,
            ...
            $wpdb->query($wpdb->prepare('UPDATE ' . $wpdb->prefix . 'term_taxonomy SET count="%d" WHERE term_id="%d"', $wpdb->get_var($wpdb->prepare('SELECT COUNT(image_id) FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE tag_id="%d"', $tag_id)), $tag_id));
          }
        }
      }
    }
  ...
  public function image_recover_all() {
    ...
    $thumb_width = $options->upload_thumb_width;
    $thumb_height = $options->upload_thumb_height;    
    $image_ids_col = $wpdb->get_col($wpdb->prepare('SELECT id FROM ' . $wpdb->prefix . 'bwge_image WHERE gallery_id="%d"', $gallery_id));
  ...
  public function image_publish() {
    $id = ((isset($_POST['image_current_id'])) ? esc_html(stripslashes($_POST['image_current_id'])) : 0);
    global $wpdb;
    $save = $wpdb->update($wpdb->prefix . 'bwge_image', array('published' => 1), array('id' => $id));
  }
  ...
  public function image_publish_all() {
    ...
    $gallery_id = ((isset($_POST['current_id'])) ? esc_html(stripslashes($_POST['current_id'])) : 0);
    if (isset($_POST['check_all_items'])) {
      $wpdb->query($wpdb->prepare('UPDATE ' .  $wpdb->prefix . 'bwge_image SET published=1 WHERE gallery_id="%d"', $gallery_id));
    }
    else {
      $image_ids_col = $wpdb->get_col($wpdb->prepare('SELECT id FROM ' . $wpdb->prefix . 'bwge_image WHERE gallery_id="%d"', $gallery_id));
      foreach ($image_ids_col as $image_id) {
        if (isset($_POST['check_' . $image_id])) {
          $wpdb->update($wpdb->prefix . 'bwge_image', array('published' => 1), array('id' => $image_id));
        }
      }
    }
  }
  ...
  public function image_unpublish() {
    $id = ((isset($_POST['image_current_id'])) ? esc_html(stripslashes($_POST['image_current_id'])) : 0);
    global $wpdb;
    $save = $wpdb->update($wpdb->prefix . 'bwge_image', array('published' => 0), array('id' => $id));
  }
  ...
  public function image_unpublish_all() {
    ...
    $gallery_id = ((isset($_POST['current_id'])) ? esc_html(stripslashes($_POST['current_id'])) : 0);
    if (isset($_POST['check_all_items'])) {
      $wpdb->query($wpdb->prepare('UPDATE ' .  $wpdb->prefix . 'bwge_image SET published=0 WHERE gallery_id="%d"', $gallery_id));
    }
    else {
      $image_ids_col = $wpdb->get_col($wpdb->prepare('SELECT id FROM ' . $wpdb->prefix . 'bwge_image WHERE gallery_id="%d"', $gallery_id));
      foreach ($image_ids_col as $image_id) {
        if (isset($_POST['check_' . $image_id])) {
          $wpdb->update($wpdb->prefix . 'bwge_image', array('published' => 0), array('id' => $image_id));
        }
      }
    }
  }
  ...
  public function image_delete() {
    $id = ((isset($_POST['image_current_id'])) ? esc_html(stripslashes($_POST['image_current_id'])) : 0);
    global $wpdb;
    $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image WHERE id="%d"', $id));
    $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image_comment WHERE image_id="%d"', $id));
    $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image_rate WHERE image_id="%d"', $id));
    $tag_ids = $wpdb->get_col($wpdb->prepare('SELECT tag_id FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE image_id="%d"', $id));
    $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE image_id="%d"', $id));
    // Increase tag count in term_taxonomy table.
    if ($tag_ids) {
  ...
  public function image_delete_all() {
    ...
    foreach ($image_ids_col as $image_id) {
      if (isset($_POST['check_' . $image_id]) || isset($_POST['check_all_items'])) {
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image WHERE id="%d"', $image_id));
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image_comment WHERE image_id="%d"', $image_id));
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image_rate WHERE image_id="%d"', $image_id));
        $tag_ids = $wpdb->get_col($wpdb->prepare('SELECT tag_id FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE image_id="%d"', $image_id));
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE image_id="%d"', $image_id));
        // Increase tag count in term_taxonomy table.
        if ($tag_ids) {
          foreach ($tag_ids as $tag_id) {
            $wpdb->query($wpdb->prepare('UPDATE ' . $wpdb->prefix . 'term_taxonomy SET count="%d" WHERE term_id="%d"', $wpdb->get_var($wpdb->prepare('SELECT COUNT(image_id) FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE tag_id="%d"', $tag_id)), $tag_id));
  ...
  public function image_set_watermark() {
    ...
    switch ($options->built_in_watermark_type) {
      ...
      case 'image':
        ...
        foreach ($images as $image) {
          if (isset($_POST['check_' . $image->id]) || isset($_POST['check_all_items'])) {
            $this->set_image_watermark(ABSPATH . $WD_BWGE_UPLOAD_DIR . $image->image_url, ABSPATH . $WD_BWGE_UPLOAD_DIR . $image->image_url, $watermark_path, $options->built_in_watermark_size, $options->built_in_watermark_size, $options->built_in_watermark_position);
          }
        }
        break;
    }
  ...
  public function image_resize() {
    ...
    foreach ($images as $image) {
      if (isset($_POST['check_' . $image->id]) || isset($_POST['check_all_items'])) {
        $this->bwge_scaled_image(ABSPATH . $WD_BWGE_UPLOAD_DIR . $image->image_url, $image_width, $image_height);
      }
    }
  }

  function bwge_scaled_image($file_path, $max_width = 0, $max_height = 0, $crop = FALSE) {
  ...
  function bwge_imagettfbboxdimensions($font_size, $font_angle, $font, $text) {
  ...
  function set_text_watermark($original_filename, $dest_filename, $watermark_text, $watermark_font, $watermark_font_size, $watermark_color, $watermark_transparency, $watermark_position) {
    ...
    $watermark_color = $this->bwge_hex2rgb($watermark_color);
    ...
    $watermark_font = WD_BWGE_DIR . '/fonts/' . $watermark_font;
    $watermark_font_size = ($height * $watermark_font_size / 500) . 'px';
    $watermark_position = explode('-', $watermark_position);
    $watermark_sizes = $this->bwge_imagettfbboxdimensions($watermark_font_size, 0, $watermark_font, $watermark_text);
  ...
  function set_image_watermark($original_filename, $dest_filename, $watermark_url, $watermark_height, $watermark_width, $watermark_position) {
    ...
    list($width, $height, $type) = getimagesize($original_filename);
    list($width_watermark, $height_watermark, $bwge_typewatermark) = getimagesize($watermark_url);
  ...
  public function save_image_db() {
    ...
    foreach ($image_id_array as $image_id) {
      if ($image_id) {
        ...
        $filetype = ((isset($_POST['input_filebwge_type' . $image_id])) ? esc_html(stripslashes($_POST['input_filebwge_type' . $image_id])) : '');
        ...
        if (strpos($image_id, 'pr_') !== FALSE) {
          $save = $wpdb->insert($wpdb->prefix . 'bwge_image', array(
            'gallery_id' => $gal_id,
            'slug' => BWGELibrary::bwge_spider_replace4byte($alt),
            'filename' => $filename,
            'image_url' => $image_url,
            'thumb_url' => $thumb_url,
            'description' => BWGELibrary::bwge_spider_replace4byte($description),
            'alt' => BWGELibrary::bwge_spider_replace4byte($alt),
            'date' => $date,
            'size' => $size,
            'filetype' => $filetype,
          ...
          $new_image_id = (int) $wpdb->get_var('SELECT MAX(`id`) FROM ' . $wpdb->prefix . 'bwge_image');
          if (isset($_POST['check_' . $image_id])) {
          ...
          $save = $wpdb->update($wpdb->prefix . 'bwge_image', array(
            'gallery_id' => $gal_id,
            'slug' => BWGELibrary::bwge_spider_replace4byte($alt),
            'filename' => $filename,
            'image_url' => $image_url,
        ...
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image_tag WHERE image_id="%d" AND gallery_id="%d"', $image_id, $gal_id));
        if ($save !== FALSE) {
  ...
  public function save() {
    $msg = __("Item Succesfully Saved.","bwge_back");  
    $not_set_items = $this->check_pricelist(); 
    if(empty($not_set_items) === false){
        $msg .= __(" Selected pricelist item longest dimension greater than some original images dimensions.","bwge_back");
    }   
    echo BWGELibrary::message($msg, 'updated');
    $this->display();
  }

  public function delete_unknown_images() {
    global $wpdb;
    $wpdb->query('DELETE FROM ' . $wpdb->prefix . 'bwge_image WHERE gallery_id=0');
  }

  public function bwge_get_unique_slug($slug, $id) {
    global $wpdb;
    $slug = sanitize_title($slug);
    if ($id != 0) {
      $query = $wpdb->prepare("SELECT slug FROM " . $wpdb->prefix . "bwge_gallery WHERE slug = %s AND id != %d", $slug, $id);
    }
    else {
      $query = $wpdb->prepare("SELECT slug FROM " . $wpdb->prefix . "bwge_gallery WHERE slug = %s", $slug);
    }
    if ($wpdb->get_var($query)) {
      ...
      do {
        $alt_slug = $slug . "-$num";
        $num++;
        $slug_check = $wpdb->get_var($wpdb->prepare("SELECT slug FROM " . $wpdb->prefix . "bwge_gallery WHERE slug = %s", $alt_slug));
      } while ($slug_check);
      $slug = $alt_slug;
    }
    return $slug;
  ...
  public function bwge_get_unique_name($name, $id) {
    global $wpdb;
    if ($id != 0) {
      $query = $wpdb->prepare("SELECT name FROM " . $wpdb->prefix . "bwge_gallery WHERE name = %s AND id != %d", $name, $id);
    }
    else {
      $query = $wpdb->prepare("SELECT name FROM " . $wpdb->prefix . "bwge_gallery WHERE name = %s", $name);
    }
    if ($wpdb->get_var($query)) {
      $num = 2;
      do {
        $alt_name = $name . "-$num";
        $num++;
        $slug_check = $wpdb->get_var($wpdb->prepare("SELECT name FROM " . $wpdb->prefix . "bwge_gallery WHERE name = %s", $alt_name));
      } while ($slug_check);
      $name = $alt_name;
    }
    return $name;
  ...
  public function save_db() {
    ...
    if ($preview_image == '') {
      if ($id != 0) {
        $random_preview_image = $wpdb->get_var($wpdb->prepare("SELECT random_preview_image FROM " . $wpdb->prefix . "bwge_gallery WHERE id='%d'", $id));
        if ($random_preview_image == '' || !file_exists(ABSPATH . $WD_BWGE_UPLOAD_DIR . $random_preview_image)) {
          $random_preview_image = $wpdb->get_var($wpdb->prepare("SELECT thumb_url FROM " . $wpdb->prefix . "bwge_image WHERE gallery_id='%d' ORDER BY `order`", $id));
        }
      }
      else {
        $i = 0;
        $random_preview_image = '';
        while (isset($_POST['thumb_url_pr_' . $i]) && isset($_POST["input_filebwge_typepr_" . $i])) {
          /*if ($_POST["input_filebwge_typepr_" . $i] == "JPG" || $_POST["input_filebwge_typepr_" . $i] == "PNG" || $_POST["input_filebwge_typepr_" . $i] == "GIF")*/ {
    ...
    if ($id != 0) {
      $save = $wpdb->update($wpdb->prefix . 'bwge_gallery', array(
        'name' => $name,
        'slug' => $slug,
      ...
      $query2 = "SELECT ID, post_content FROM " . $wpdb->posts . " WHERE post_type = 'bwge_gallery'";
      ...
      $save = $wpdb->insert($wpdb->prefix . 'bwge_gallery', array(
        'name' => $name,
        'slug' => $slug,
        'description' => $description,
        'page_link' => '',
        'preview_image' => $preview_image,
        'random_preview_image' => $random_preview_image,
        'order' => ((int) $wpdb->get_var('SELECT MAX(`order`) FROM ' . $wpdb->prefix . 'bwge_gallery')) + 1,
        'author' => get_current_user_id(),
        'gallery_type' => $gallery_type,
        'gallery_source' => $gallery_source,
        'autogallery_image_number' => $autogallery_image_number,
    ...
    if ($save !== FALSE) {
      echo BWGELibrary::message(__('Item Succesfully Saved.', 'bwge_back'), 'updated');
    }
    else {
      echo BWGELibrary::message(__('Error. Please install plugin again.', 'bwge_back'), 'error');
    }
  }

  public function save_order($flag = TRUE) {
    global $wpdb;
    $gallery_ids_col = $wpdb->get_col('SELECT id FROM ' . $wpdb->prefix . 'bwge_gallery');
    if ($gallery_ids_col) {
      foreach ($gallery_ids_col as $gallery_id) {
        if (isset($_POST['order_input_' . $gallery_id])) {
          ...
          $order_values[$gallery_id] = (int) $wpdb->get_var($wpdb->prepare('SELECT `order` FROM ' . $wpdb->prefix . 'bwge_gallery WHERE `id`="%d"', $gallery_id));
        }
      }
      asort($order_values);
      $i = 1;
      foreach ($order_values as $key => $order_value) {
        $wpdb->update($wpdb->prefix . 'bwge_gallery', array('order' => $i), array('id' => $key));
        $i++;
      }
      if ($flag) {
        echo BWGELibrary::message(__('Ordering Succesfully Saved.', 'bwge_back'), 'updated');
      }
    }
    $this->display();
  }
  ...
  public function delete($id) {
    global $wpdb;
    $query = $wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_gallery WHERE id="%d"', $id);
    $query_image = $wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image WHERE gallery_id="%d"', $id);
    $query_album_gallery = $wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_album_gallery WHERE alb_gal_id="%d" AND is_album="%d"', $id, 0);
    if ($wpdb->query($query)) {
      $wpdb->query($query_image);
      $wpdb->query($query_album_gallery);
      echo BWGELibrary::message(__('Item Succesfully Deleted.', 'bwge_back'), 'updated');
    }
    else {
      echo BWGELibrary::message(__('Error. Please install plugin again.', 'bwge_back'), 'error');
    }
  ...
  public function delete_all() {
    global $wpdb;
    $flag = FALSE;
    $gal_ids_col = $wpdb->get_col('SELECT id FROM ' . $wpdb->prefix . 'bwge_gallery');
    foreach ($gal_ids_col as $gal_id) {
      if (isset($_POST['check_' . $gal_id]) || isset($_POST['check_all_items'])) {
        $flag = TRUE;
        $query = $wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_gallery WHERE id="%d"', $gal_id);
        $wpdb->query($query);
        $query_image = $wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_image WHERE gallery_id="%d"', $gal_id);
        $wpdb->query($query_image);
        $query_album_gallery = $wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'bwge_album_gallery WHERE alb_gal_id="%d" AND is_album="%d"', $gal_id, 0);
        $wpdb->query($query_album_gallery);
      }
    }
    if ($flag) {
      echo BWGELibrary::message(__('Items Succesfully Deleted.', 'bwge_back'), 'updated');
    }
    else {
  ...
  public function publish($id) {
    global $wpdb;
    $save = $wpdb->update($wpdb->prefix . 'bwge_gallery', array('published' => 1), array('id' => $id));
    if ($save !== FALSE) {
      echo BWGELibrary::message(__('Item Succesfully Published.', 'bwge_back'), 'updated');
    }
    else {
      echo BWGELibrary::message(__('Error. Please install plugin again.', 'bwge_back'), 'error');
    }
  ...
  public function publish_all() {
    ...
    $flag = FALSE;
    if (isset($_POST['check_all_items'])) {
      $wpdb->query('UPDATE ' .  $wpdb->prefix . 'bwge_gallery SET published=1');
      $flag = TRUE;
    }
    else {
      $gal_ids_col = $wpdb->get_col('SELECT id FROM ' . $wpdb->prefix . 'bwge_gallery');
      foreach ($gal_ids_col as $gal_id) {
        if (isset($_POST['check_' . $gal_id])) {
          $flag = TRUE;
          $wpdb->update($wpdb->prefix . 'bwge_gallery', array('published' => 1), array('id' => $gal_id));
        }
      }
    }
    if ($flag) {
  ...
  public function unpublish($id) {
    global $wpdb;
    $save = $wpdb->update($wpdb->prefix . 'bwge_gallery', array('published' => 0), array('id' => $id));
    if ($save !== FALSE) {
      echo BWGELibrary::message(__('Item Succesfully Unpublished.', 'bwge_back'), 'updated');
    }
    else {
      echo BWGELibrary::message(__('Error. Please install plugin again.', 'bwge_back'), 'error');
    }
  ...
  public function unpublish_all() {
    ...
    $flag = FALSE;
    if (isset($_POST['check_all_items'])) {
      $wpdb->query('UPDATE ' .  $wpdb->prefix . 'bwge_gallery SET published=0');
      $flag = TRUE;
    }
    else {
      $gal_ids_col = $wpdb->get_col('SELECT id FROM ' . $wpdb->prefix . 'bwge_gallery');
      foreach ($gal_ids_col as $gal_id) {
        if (isset($_POST['check_' . $gal_id])) {
          $flag = TRUE;
          $wpdb->update($wpdb->prefix . 'bwge_gallery', array('published' => 0), array('id' => $gal_id));
        }
      }
    }
    if ($flag) {
  ...
  public function resize_image_thumb() {
    ...
    $flag = FALSE;
    $gallery_id = ((isset($_POST['current_id'])) ? esc_html(stripslashes($_POST['current_id'])) : 0);
    $img_ids = $wpdb->get_results($wpdb->prepare('SELECT id, thumb_url FROM ' . $wpdb->prefix . 'bwge_image WHERE gallery_id="%d"', $gallery_id));
    $options = $wpdb->get_row('SELECT * FROM ' . $wpdb->prefix . 'bwge_option');
    foreach ($img_ids as $img_id) {
      if (isset($_POST['check_' . $img_id->id]) || isset($_POST['check_all_items'])) {
	      $flag = TRUE;
        $file_path = str_replace("thumb", ".original", htmlspecialchars_decode(ABSPATH . $WD_BWGE_UPLOAD_DIR . $img_id->thumb_url, ENT_COMPAT | ENT_QUOTES));
	      $new_file_path = htmlspecialchars_decode(ABSPATH . $WD_BWGE_UPLOAD_DIR . $img_id->thumb_url, ENT_COMPAT | ENT_QUOTES);
      ...
      echo BWGELibrary::message(__('You must select at least one item.', 'bwge_back'), 'error');
		  ...
		  echo BWGELibrary::message(__('Thumb Succesfully Resized', 'bwge_back'), 'updated');
	  }
  }
  ...
  public function set_image_pricelist(){
    ...
    foreach ($image_ids_col as $image_id) {
      if (isset($_POST['check_' . $image_id]) || isset($_POST['check_all_items'])) {
		...
		if($pricelist_id){
		  $file_path = str_replace("thumb",".original", htmlspecialchars_decode( ABSPATH . $WD_BWGE_UPLOAD_DIR .$_POST['thumb_url_' . $image_id], ENT_COMPAT | ENT_QUOTES));
		...
		$wpdb->update($wpdb->prefix . 'bwge_image', array('pricelist_id' => $pricelist_id), array('id' => $image_id));

      }
    }

	if(empty( $not_set_items) === false){
	  echo "<div class='bwge_msg'>".__('Selected pricelist item longest dimension greater than some original images dimensions.', 'bwge_back') ."</div>";  
	}
  ...
  public function check_pricelist(){
    ...
    $not_set_items = array();
    if($gallery_id){
       $rows = $wpdb->get_results('SELECT T_IMAGES.thumb_url, T_PRICELISTS.item_longest_dimension, T_IMAGES.id FROM ' . $wpdb->prefix . 'bwge_image AS T_IMAGES LEFT JOIN ( SELECT  MAX(item_longest_dimension) AS item_longest_dimension, pricelist_id FROM ' . $wpdb->prefix . 'bwge_pricelist_items AS T_PRICELIST_ITEMS LEFT JOIN '. $wpdb->prefix . 'bwge_pricelists AS T_PRICELISTS ON T_PRICELIST_ITEMS.pricelist_id = T_PRICELISTS.id  WHERE  T_PRICELISTS.sections LIKE "%downloads%" GROUP BY pricelist_id) AS T_PRICELISTS ON T_IMAGES.pricelist_id = T_PRICELISTS.pricelist_id WHERE T_IMAGES.gallery_id="'.$gallery_id.'"');
      
       foreach($rows as $row){
          if($row->item_longest_dimension){  
              $file_path = str_replace("thumb",".original", htmlspecialchars_decode( ABSPATH . $WD_BWGE_UPLOAD_DIR .$row->thumb_url, ENT_COMPAT | ENT_QUOTES));
  ...
  public function remove_image_pricelist(){
	...
	if($image_id){
	  $wpdb->update($wpdb->prefix . 'bwge_image', array('pricelist_id' => 0), array('id' => $image_id));
	}
  }
